...
> $(|tx|)^{k}/k! \leq e^{|tx|} \leq e^{tx} + e^{-tx}$

양 끝변에 기댓값을 취하면

> $ |t|^{k}E(|X|^{k})/k! \leq M(t) + M(-t) < \infty \ (-h < t < h)$
...
확률변수 $X$에 대하여 $E(|X|^{s}) < \infty$이면 $0 < r < s$ 인 $r$에 대해서
...
여기서 $p=s/r>1, Y=|X|^{r}$이라고 하면

> $ E[Y] = E[|X|^{r}(I_{|X| \leq 1} + I_{|X|>1})] \leq E[1+|X|^{s}] < \infty $\\
$ (E[|X|^{r}])^{s/r} = (E[Y])^{p} \leq E(Y^{p}) = E[(|X|^{r})^{s/r}]=E[|X|^{s}] $

### Markov inequality

확률변수 $Z$에 대하여 $E(|Z|^{r}) < \infty, (r>0)$이면 임의의 양수 $k$에 대하여는

> $P(|Z| \geq k) \leq E(|Z|^{r})/k^{r}$

**증명**

$P(|Z| \geq k)=E(I_{|Z| \geq k})$ 이다. 그리고 지표함수는 0 또는 1이므로 $I_{(|Z| \geq k)} = 1 \times I_{(|Z|/k \geq 1)} \leq (|Z|/k)^{r} \times I_{(|Z|/k \geq 1)} \leq |Z|^{r}/k^{r}$ 따라서 양변에 기댓값을 취하면:

> $P(|Z| \geq k)=E(I_{|Z| \geq k}) \leq E(|Z|^{r})/k^{r}$
...
> $P(|X-\mu| \geq 6\sigma) \leq 1/36 \approx 0.0277$
