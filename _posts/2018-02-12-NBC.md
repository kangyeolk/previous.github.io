...
첫 번째 줄은 **베이즈 정리** 에 의해 알 수 있습니다. 또한 분모의 $$P(x_{1},...,x_{p})$$ 부분은 모든 $$C_{k}$$에 대해서 공통으므로 날릴 수 있습니다. 그리고 **$$C_{k}$$가 주어 졌을 때 $$(x_{1},...,x_{p})$$의 조건부 독립** 을 가정한다면, 마지막까지 계산할 수 있습니다. 이제 $$P(x_{1} \lvert C_{k}),...,P(x_{p} \lvert C_{k}),P(C_{k})$$를 계산해야 합니다. 하지만 그를 위해서는 먼저 MLE 방식으로 각 모수를 구해야 합니다.
