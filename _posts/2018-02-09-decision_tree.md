...
여기서 Information gain은 원래의 부모 노드와 자식 노드들 사이의 엔트로피의 차로써 구해진다, a를 분리 기준이라고 한다면:
$$IG(T,a)[Information Gain] = H(T)[Entropy(parent)] - H(T|a)[Weighted Sum of Entropy(Children)]$$
...
이러한 지니 불순도가 가장 낮아지는 변수-최적 분리점을 찾아서 그 기준으로 분리를 하면 됩니다. 범주형은 쉽게 계산되지만, 연속형 변수의 경우에는 분리점 몇 개를 지정해서 그걸 가지고 분리해 봅니다. 계산량이 너무 많기 때문이죠. 정리하면:
각 변수-최적 분리점으로 분리 했을 때의 지니 불순도를 계산하여 그 중 지니 불순도가 제일 낮아지는 변수-분리기준을 택해서 분리합니다.
