...
* 왜 위와 같은 결과가 나오냐면 $$0 \leq \alpha < 1/8$$라는 범위에서는 $$T_{t}=T_{t_{2}}$$이라고 할 때, $$\alpha \leq \frac{R(t)-R(T_{t})}{\lvert f(T_{t})\rvert-1}$$가 되고, $$T_{t}=T_{t_{1}}$$이라고 할 때, $$\alpha > \frac{R(t)-R(T_{t})}{\lvert f(T_{t}) \rvert-1}$$이기 때문입니다. 이 점에서 위와 같은 범위에서 $$T_{t_{1}}$$이 $$T_{t_{2}}$$보다 좋은 것은 명확합니다. 마찬가지로 $$T_{t_{1}}$$이 $$T_{t_{3}}$$보다 더 좋은 것도 명확합니다. 즉, 해당 t로 인한 $$g(t)$$의 값이 지정된 $$\alpha$$보다 커지면 가지치기를 하지 않는 것이 좋습니다.
